document.addEventListener('alpine:init', () => {
    Alpine.data('calculator', () => ({
        ...
        init() {
            ...
            this.$watch('inspectorYear', () => this.updateCharts()); // NEW: Watch slider
        ...
        getNW(s, idx) {
            ...
            return arr ? (arr[idx] || 0) : 0;
                ...
                get breakEven() {
                    ...
                    // Find first year where Buy > Rent

                    // Note: Year 0 is index 0. We display "Year X".
            ...
            // 2. Select Baseline (Reference Point)
            // If Winner is Rent -> Baseline is Best Buy (Runner Up)
            // If Winner is Buy -> Baseline is Rent
            let baseline = null;
            if (winner.type === 'Rent') {
                baseline = runnerUp; // The best buying option
            } else {
                baseline = strats.find(s => s.type === 'Rent'); // Always Rent
            }
            ...
            // Part A: The Summary (Anchor to Baseline)
            if (baseline) {
            ...
            // Part B: The Breakdown (Winner Analysis)
            ...
            // Part C: The Nuance (Runner Up Check)
            // If the Runner-up isn't the Baseline (e.g. Winner=Lodger, Baseline=Rent, RunnerUp=BuyHome),
            // we should mention why Lodger beat BuyHome.
        ...
        get tableData() {
            ...
            const years = [2, 5, 10, 15, 25, 40]; const yearStats = {};
            years.forEach(y => { const vals = strategies.map(str => this.getNW(str.s, y-1)); yearStats[y] = { max: Math.max(...vals), min: Math.min(...vals), count: vals.length }; });
            return strategies.map(str => {
                const cells = years.map(y => {
                    const val = this.getNW(str.s, y-1); const stats = yearStats[y]; let style = "text-gray-600";
        ...
        get insights() {
            const tips = []; if (!this.i.home.active) return tips;
        ...
        calculate() {
            try { this.results = Engine.simulateStrategies(this.payload); this.updateCharts(); } catch(e) { console.error("Calc Error", e); }
        },
        ...
        applyPreset(name) {
            const presets = {
                nomad: { personal: { liquidAssets: 100000, rent: {current:800}, stockGrowth: 12, monthlySavings: 2000, isFTB: true }, home: { active: false }, btl: { active: false } },
                london: { personal: { liquidAssets: 130000, isaBalance: 30000, monthlySavings: 1500, taxBand: 'higher', rent: {current:2400}, isFTB: true }, home: { active: true, price: 500000, depositPct: 20, rate: 4.5, serviceCharge: 2000, repairRate: 0.5 }, btl: { active: false } },
                starter: { personal: { liquidAssets: 40000, isaBalance: 5000, monthlySavings: 800, stockGrowth: 8, rent: {current:1200}, isFTB: true, taxBand: 'basic' }, home: { active: true, price: 280000, depositPct: 10, rate: 4.8, renoCost: 5000, lodger: {active: true, income: 625, years: 5} }, btl: { active: false } },
                landlord: { personal: { liquidAssets: 90000, isaBalance: 10000, monthlySavings: 3000, stockGrowth: 8, rent: {current:2500}, isFTB: false, taxBand: 'additional' }, home: { active: false }, btl: { active: true, price: 200000, depositPct: 25, rentYield: 6, wrappers: { company: true, personal: false } } }
            };
        ...
        save() { localStorage.setItem('rentVsBuyData_v3', JSON.stringify(this.i)); }
    }));
});